...
test('create', () => {
  const problemDocument = new ProblemDocument({ detail: 'a', instance: 'b', status: 400, title: 'd', type: 'e' });
...
test('createWithExtension', () => {
  const problemDocument = new ProblemDocument({ detail: 'a', instance: 'b', status: StatusCodes.BAD_REQUEST, title: 'd', type: 'e', foo: 'bar' });
  expect(problemDocument).toBeInstanceOf(ProblemDocument);
  const { detail, instance, status, title, type, ...other } = problemDocument;
  expect(detail).toBe('a');
  expect(instance).toBe('b');
